...
def load_model():
    ...
    model = keras.models.load_model(os.path.join(MODEL_PATH, MODEL_NAME), compile=False) # customize loss function
    ...
    def record_log(log_msg):
        if not os.path.isfile(LOG_PATH + '\\log.csv'):
            with open(LOG_PATH + '\\log.csv', 'w',newline='') as csv_file:
                ...
                writer.writerow(['FileName', 'Class'] + CLASSES)
                csv_file.flush()
    ...
    for root, dirs, files in os.walk(IMAGE_PATH):
        for file in files:
            if file.endswith(tuple(EXT)):
                ...
                log_msg = [file, CLASSES[top_inds[0]]] + scores
                record_log(log_msg)
