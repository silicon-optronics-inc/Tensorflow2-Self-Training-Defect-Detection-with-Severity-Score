...
def predict_cls_img(cls_model, img, obj_boxes, obj_scores, obj_classes, show_box_num):
    '''Obtain information such as defect type and coordinates from the object detection model, and then crop the defect and send it to the classification model to obtain a severity score.
        Scores from object detection are used directly if severity is not interested.
    Args:
        cls_model: keras classification model
        img: numpy.ndarray, image
        obj_boxes: numpy.ndarray, contains predicted boxes coordinates, ([[xmin1 ymin1 xmax1 ymax1], [xmin2 ymin2 xmax2 ymax2], ....])
        obj_scores: numpy.ndarray, confidence score of each predicted box, ([score1 score2 score3...])
        obj_classes: numpy.ndarray, predicted classes of each object, ([class1 class2 class3...])
        show_box_num: int, number of defects to be shown 
        
    Returns:
        cls_infor: list, information of defect type, score, coordinates, ([[class1, score1, xmin1 ymin1 xmax1 ymax1], [class2, score2, ...], ...])
    ...
    for i in range(min(len(obj_boxes), show_box_num)):
        ...
        if obj_classes[i] == 1:
            # crop dashcol image for classification
            ...
            score = str('%.4f'%score)
            cls_infor.append([obj_classes[i], score, 0, xmin, OBJ_MODEL_HEIGHT, xmax])
        elif obj_classes[i] == 2:
            score = str('%.4f'%obj_scores[i])
...
def process_log(file, img, cls_infor, pbtxt_dict):
    ...
    for i in range(len(cls_infor)):
        ...
        img = draw_bndbox(img, classid, cls_infor[i][1], cls_infor[i][2], cls_infor[i][3], cls_infor[i][4], cls_infor[i][5], pbtxt_dict, colour, i)
...
if __name__ == '__main__':
    ...
    for root, dirs, files in os.walk(IMG_PATH):
        for file in files:
            ...
            # classification
            cls_infor = predict_cls_img(cls_model, img, obj_boxes, obj_scores, obj_classes, show_box_num)
